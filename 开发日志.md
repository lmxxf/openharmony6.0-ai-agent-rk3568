...
### 10. 添加模型选择菜单 (2025-12-21 19:00)

*   **功能：** 点击"切换模型"按钮可选择不同模型
*   **支持模型：**
    *   Qwen2.5-0.5B (470MB)
    *   Qwen3-0.6B (381MB)
*   **UI 实现：** 使用 ArkUI 的 `bindMenu` + `Menu` + `MenuItem` 组件

---

### 11. 待解决：推理无响应问题 (2025-12-21 19:15)

*   **现象：** 模型加载成功，发送消息后一直显示"AI正在思考..."，无输出
*   **日志显示：** `Tokenized to 39 tokens` 后无后续日志
*   **可能原因：**
    1. RK3568 A55 CPU 推理太慢（0.5 tokens/s，256 tokens 需要 8+ 分钟）
    2. 推理线程卡死
    3. 异步回调未正确触发
*   **待排查**
...
### 13. 云端推理模式：DeepSeek API 接入 (2026-01-28)

*   **背景：** RK3568 本地推理太慢（0.5 tokens/s），添加云端 API 模式作为替代方案

*   **架构改动：**
    ```
    aiAssistant.ets
    ├── 本地模式 → llama_napi.so → Qwen3-0.6B
    └── 云端模式 → HTTP 请求 → DeepSeek API（流式输出）
    ```

*   **代码修改：**
    *   `aiAssistant.ets`: 新增推理模式切换（本地/云端），默认云端
    *   使用 `@ohos.net.http` 模块发起 HTTPS 请求
    *   支持 SSE 流式输出，实时显示 AI 回复
    *   API Key 从 `rawfile/api_config.json` 读取（已 gitignore）

*   **权限变更：**
    *   `module.json5`: 新增 `ohos.permission.INTERNET` 权限

*   **配置文件：**
    *   `rawfile/api_config.json` - 存放 API Key（不提交）
    *   `rawfile/api_config.json.example` - 示例配置（提交）

*   **ArkTS 类型严格性踩坑：**
    *   ArkTS 不允许 `object[]` 类型的数组字面量
    *   必须定义 `interface ApiMessage` 并显式声明类型
    *   对象字面量必须对应已声明的接口

*   **UI 改进：**
    *   状态栏新增"本地/云端"切换按钮
    *   云端模式隐藏"切换模型"按钮（只有一个 DeepSeek）
    *   欢迎界面文案根据模式动态显示

*   **图标修复：**
    *   原 `ic_ai.svg` 使用 `stroke="currentColor"` 无填充色，在设置列表不显示
    *   重写为带 `fill="#7C4DFF"` 的紫色机器人图标
    *   图标位置：`src/main/ets/res/image/ic_ai.svg`（不是 `resources/base/media/`）
...
*   **双模式支持：**
    *   本地推理：Qwen3-0.6B / Qwen2.5-0.5B（离线可用，慢）
    *   云端推理：DeepSeek Chat API（需联网，快）
*   **默认模式：** 云端（因为本地太慢）
*   **已验证：** 云端 API 调用成功，流式输出正常

---

### 14. Demo 功能：Settings 内部 UI-Agent（蓝牙/亮度/日期时间）（2026-01-28）

*   **目标：** 不依赖无障碍、不跨应用；在“系统设置 App 内”实现一句话直达/直改的 UI 操作 demo。
*   **实现方式：**
    *   在 `aiAssistant.ets` 内增加本地白名单动作解析与执行器（UiDriver），命中关键词即直接执行；未命中则走 DeepSeek 正常聊天。
    *   只开放 3 个能力点（避免 Wi‑Fi 断网影响云端演示）。
*   **支持命令示例：**
    *   蓝牙：`打开蓝牙` / `关闭蓝牙` / `切换蓝牙`（或 `蓝牙开关`）→ 实际调用系统蓝牙 enable/disable，并跳转到 `pages/bluetooth`
    *   亮度：`亮度 35%` / `亮度35` / `设置亮度 80` → 映射设备 min/max 后设置系统亮度，并跳转到 `pages/screenAndBrightness`
    *   日期时间：`切到24小时制` / `切到12小时制` → 修改系统时间制式，并跳转到 `pages/dateAndTime`
*   **备注：**
    *   这是 Demo 版“直接执行”模式；后续可升级为 “LLM 生成 JSON steps → 严格校验后执行” 的可扩展架构。
